...
open class HelpEvaluation : Evaluator {
    ...
    override fun evaluate(tree: Parser.ASTNode, output: PrintStream, state: EvaluationState) {
        ...
        output.println("[some expression] (mod p) (field d(X))")
        ...
        output.println("isprimitive [p(X)] (mod p) (field d(X))")
        output.println("- Checks if p(X) is primitive in a given field.")
        output.println("findprimitives (mod p) (field d(X))")
        ...
        output.println("findirreducibles n (mod p)")
